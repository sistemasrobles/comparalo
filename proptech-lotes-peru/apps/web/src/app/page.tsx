...
import { formatPrice, getLegalStatusLabel, getLegalStatusColor, type ProjectCategory } from '@/lib/projects-data';
import { getHeroSlides, type HeroSlide } from '@/lib/admin-store';
import { useAdminProjects } from '@/lib/hooks/useAdminProjects';
import { PromoBanner } from '@/components/PromoBanner';
import { ProjectAdBanners } from '@/components/ProjectAdBanners';
...
function ExclusiveCarousel({ exclusive }: { exclusive: ProjectData[] }) {
  ...
  const getCardWidth = () => {
    ...
    // Mirror Tailwind breakpoints: sm=640px → 2 cols, lg=1024px → 3 cols
  ...
  const goTo = (idx: number) => {
    ...
    setCurrent(clamped);
    setOffset(getCardWidth() * clamped);
  };

  // Recalculate offset on resize
  useEffect(() => {
    const handler = () => setOffset(getCardWidth() * current);
  ...
  return (
    <section className="py-16 md:py-20 bg-gradient-to-br from-amber-50/80 via-white to-amber-50/40 border-y border-amber-100/50 overflow-hidden">
        ...
        <div className="relative px-6">
          ...
          {total > 1 && (
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md border border-amber-100 flex items-center justify-center text-amber-700 hover:bg-amber-50 transition-colors"
              aria-label="Anterior"
            >
              <IconChevronLeft />
            </button>
          ...
          {/* Cards wrapper — overflow hidden clips the cards outside view */}
          <div className="overflow-hidden" ref={trackRef}>
            {/* 
              Each card: w-full (mobile), w-1/2 (sm), w-1/3 (lg)
              translateX by (current * cardWidthPercent).
              On mobile card = 100%, sm = 50%, lg = 33.33%.
              We use a CSS custom property trick via inline style on the flex row.
            */}
            <div
              ...
              {exclusive.map((p) => (
                ...
                >
                  <Link
                    ...
                    className="group relative card-interactive overflow-hidden bg-white ring-1 ring-amber-200/60 hover:ring-amber-300 block"
                  >
                    ...
                    <div className="p-4">
                      <div className="flex items-baseline justify-between mb-3">
                        <span className="text-lg font-bold text-primary-600">{formatPrice(p.minPrice, p.currency ?? 'PEN')}</span>
                      </div>
          ...
          {total > 1 && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md border border-amber-100 flex items-center justify-center text-amber-700 hover:bg-amber-50 transition-colors"
            ...
            {exclusive.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  i === current ? 'bg-amber-500 w-5' : 'bg-amber-200 hover:bg-amber-300 w-2'
                }`}
...
export default function HomePage() {
  // useAdminProjects: reactivo a cambios del admin en tiempo real
  // (escucha evento 'storage' del browser + foco de ventana)
  const allProjects = useAdminProjects();

  const featured = useMemo(() => allProjects.filter((p) => p.isFeatured), [allProjects]);
  const exclusive = useMemo(() => allProjects.filter((p) => p.isExclusive), [allProjects]);
  const cities = useMemo(() => [...new Set(allProjects.map((p) => p.city))].filter(Boolean).sort(), [allProjects]);
  const totalLots = useMemo(() => allProjects.reduce((sum, p) => sum + (p.totalLots ?? 0), 0), [allProjects]);
  ...
  return (
    <div>
        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/15 border border-white/30 text-xs font-medium text-white mb-5 backdrop-blur-sm">
              ...
              {allProjects.length} proyectos verificados en {cities.length} ciudades
            </div>
      ...
      {/* ═══════════════════════════════════════════
          BANNERS DE PROYECTOS — Estilo Nexo
      ═══════════════════════════════════════════ */}
      ...
      {/* ═══════════════════════════════════════════
          4.5 PROYECTOS EXCLUSIVOS — Carrusel
      ═══════════════════════════════════════════ */}
      <ExclusiveCarousel exclusive={exclusive} />
      ...
      <section className="py-14 bg-slate-50/80 border-y border-slate-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {cities.map((city) => {
              const cp = allProjects.filter((p) => p.city === city);
              const cheapest = cp.reduce((a, b) => a.minPrice < b.minPrice ? a : b, cp[0]);
              const minP = cheapest?.minPrice ?? 0;
              const minCurrency = cheapest?.currency ?? 'PEN';
              return (
                <Link
                  key={city}
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{cp.length} proy. · <span className="text-primary-600 font-semibold">{formatPrice(minP, minCurrency)}</span></p>
                  </div>
                </Link>
              );
            })}
              ...
              <Link key={p.id} href={`/proyecto/${p.slug}`} className="card-interactive group overflow-hidden">
                <div className="relative aspect-[16/10] bg-slate-100 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 flex gap-1.5">
                    <span className="flex items-center gap-1 bg-gradient-to-r from-primary-600 to-primary-500 text-white text-2xs font-bold px-2.5 py-1 rounded-full shadow-md uppercase tracking-wider">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-baseline justify-between">
                    <div>
                      <span className="text-base font-bold text-slate-900">{formatPrice(p.minPrice, p.currency ?? 'PEN')}</span>
      ...
      {([
        ...
        const items = allProjects.filter((p) => p.category === cat.category);
